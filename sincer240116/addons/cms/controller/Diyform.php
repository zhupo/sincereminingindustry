        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if($config['autosend'] == 1){
                    $content = 'Name：'.$row['name'].'<br> Email：'.$row['email'].'<br> Phone：'.$row['telephone'].'<br> message：'.$row['content'];
                    $email = new \app\common\library\Email;
                    $result = $email
                        ->to($config['email'])//接收邮箱
                        ->subject("您的网站ozzsmart.com.au有新的留言消息，请注意查收！")
                        ->message($content)
                        ->send();
                }
